...
TimePlay.prototype.hoverPopup = function(){
	...
	$(window).on('mousemove',function(event){
		...
		if(timePlay.timeUnit == '天'){
			...
			var texts =timePlay.temp_day.mon + '月'+ timePlay.temp_day.day + '日'+ timePlay.temp_day.hour + ":00";
		}
		//鼠标移入是显示时间,灰色的
...
TimePlay.prototype.changeTimeUnit = function(num){
	...
	if(timePlay.timeUnit == '天'){
		...
		var texts = timePlay.curr_day.mon + '月'+ timePlay.curr_day.day + '日'+timePlay.curr_day.hour + timePlay.curr_day.minute + timePlay.curr_day.second;
	}
...
TimePlay.prototype.startPlay = function(){
	...
	timePlay.timer = setInterval(function(){
		...
		if(timePlay.timeUnit == "时"){
			...
			$(".curr-popup").text(timePlay.curr_day.mon + '月'+ timePlay.curr_day.day + '日'+timePlay.curr_day.hour+":00")
		}else{
			timePlay.index++;
